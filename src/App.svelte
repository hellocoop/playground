...
<main class="p-4 space-y-4 flex-1 overflow-y-auto">
  <section class="border border-charcoal dark:border-gray-800 rounded-sm w-full p-4 flex items-start flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-10">
    <div class="w-full lg:w-1/4 lg:max-w-sm lg:min-w-[20rem]">
      ...
      <div class="bg-gray-200 dark:bg-charcoal rounded-sm p-4 break-words my-6">
        ...
        <span class="mt-2 block text-sm whitespace-pre-line" class:flash={copyTooltip.requestURL}>
          {requestURL}
        </span>
      </div>
  ...
  {#if result.authorize}  
    <section class="btn group">
      <button on:click={()=>dropdown.authorize=!dropdown.authorize} class="py-2 w-full flex justify-between items-center px-4"
        >
        <div class="flex flex-col items-start">
          <span class="font-semibold text-lg">{new URL('/authorize', states.selected_authorization_server)}</span>
    ...
    {#if result.token !== null}
      <section class="btn group">
        <button on:click={()=>dropdown.token=!dropdown.token} class="py-2 w-full flex justify-between items-center px-4">
          <div class="flex flex-col items-start">
            <span class="font-semibold text-lg">{new URL('/oauth/token', states.selected_authorization_server)}</span>
            <div class="inline-flex items-center">
              <span>Response</span>
              <button on:click={()=>{
    ...
    {#if result.userinfo !== null}
      <section class="btn group">
        <button on:click={()=>dropdown.userinfo=!dropdown.userinfo} class="py-2 w-full flex justify-between items-center px-4">
          <div class="flex flex-col items-start">
            <span class="font-semibold text-lg">{new URL('/oauth/userinfo', states.selected_authorization_server)}</span>
            <div class="inline-flex items-center">
              <span>Response</span>
              <button on:click={()=>{
                if(!result.token) return
    ...
    {#if result.introspect !== null}
      <section class="btn group">
        <button on:click={()=>dropdown.introspect=!dropdown.introspect} class="py-2 w-full flex justify-between items-center px-4">
          <div class="flex flex-col items-start">
            <span class="font-semibold text-lg">{new URL('/oauth/introspect', states.selected_authorization_server)}</span>
            <div class="inline-flex items-center">
              <span>Response</span>
              <button on:click={()=>{
    ...
    {#if result.introspect}
      <section class="btn group">
        <button on:click={()=>dropdown.claims=!dropdown.claims} class="h-12 w-full flex justify-between items-center px-4"
        >
        ...
        {#if dropdown.claims}
          <ul class="flex flex-col px-4 divide-y divide-black/50 dark:divide-white/50">
              ...
              <li class="py-4 flex items-center w-full">
                ...
                <div>
                  {#if claim === 'picture' && result.introspect[claim]}
                    ...
                    <img src={result.introspect[claim]} class="h-10 w-10 rounded-full object-fit" alt="Picture claim"/>
                  {:else}
  ...
  {:else}
    <section class="w-auto border border-charcoal dark:border-gray-800 opacity-80 h-72 flex items-center justify-center">
